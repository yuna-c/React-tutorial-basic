						...
						<dl data-form="wrap" class="noline">
							...
							<dd>
								<p>
                                    ...
                                    <span data-form="radio" class="type1">
										...
										<label for="radio4">부엉이관</label>
									</span>
                                    -->
									<span data-form="radio" class="type1">
										<input type="radio" id="radio4" name="CGD_CLASS_KBN" value="3" <%=isChecked(CGD_CLASS_KBN, "3")%> <%=iif(CInt(CGD_REP_STAT) > 1," disabled='disalbed'","" )%>>
										<label for="radio4">N관</label>
									</span>
									<span data-form="radio" class="type1">
										<input type="radio" id="radio11" name="CGD_CLASS_KBN" value="4" <%=isChecked(CGD_CLASS_KBN, "4")%> <%=iif(CInt(CGD_REP_STAT) > 1," disabled='disalbed'","" )%>>
										<label for="radio11">M관</label>
									</span>
									<span data-form="radio" class="type1">
										<input type="radio" id="radio12" name="CGD_CLASS_KBN" value="5" <%=isChecked(CGD_CLASS_KBN, "5")%> <%=iif(CInt(CGD_REP_STAT) > 1," disabled='disalbed'","" )%>>
										<label for="radio12">목동관</label>
									</span>
								</p>
							</dd>
							...
							<dd>
								...
								<p>
									...
									<span data-form="input"><input title="국어" type="text" id="CGD_KOR_SCR" name="CGD_KOR_SCR" value="<%=CGD_KOR_SCR%>" numberonly maxlength="3" <%=iif(CInt(CGD_REP_STAT) > 1, " readonly='readonly'", "" )%> onkeyup="IfSCRChk(100,this)"></span>
								</p>
								<div class="m_b30">
									<span class="field_name m_b10">수학</span>
									<p class="m_b20">
										<span data-form="radio">
											<input type="radio" id="radio5"  value="가형" name="CGD_MATH_FLG" <%=iif(CGD_MATH_FLG = "가형", "checked", "") %> <%=iif(CInt(CGD_REP_STAT) > 1, " onclick ='return false;'", "" )%>>
											<label for="radio5">가형</label>
										</span>
										<span data-form="radio">
											<input type="radio" id="radio6" value="나형" name="CGD_MATH_FLG" <%=iif(CGD_MATH_FLG = "나형", "checked", "") %> <%=iif(CInt(CGD_REP_STAT) > 1, " onclick ='return false;'", "" )%>>
											<label for="radio6">나형</label>
										</span>
									</p>
									<span data-form="input"><input title="수리영역" type="text" id="CGD_MATH_SCR" name="CGD_MATH_SCR" value="<%=CGD_MATH_SCR%>" numberonly maxlength="3" <%=iif(CInt(CGD_REP_STAT) > 1, " readonly='readonly'", "" )%> onkeyup="IfSCRChk(100,this)"></span>
								</div>
								<p class="m_b30">
								...
								<div class="m_b30">
									<p class="m_b20">
										<span data-form="radio">
											<input type="radio" id="radio7" name="CGD_SUB_FLG" value="과학"  <%=iif(CGD_SUB_FLG = "과학", "checked", "") %> <%=iif(CInt(CGD_REP_STAT) > 1, " onclick ='return false;'", " onclick=""IfSubSel('SC')"" " )%>>
											<label for="radio7">과학</label>
										</span>
										<span data-form="radio">
											<input type="radio" id="radio8" name="CGD_SUB_FLG" value="사회" <%=iif(CGD_SUB_FLG = "사회", "checked", "") %> <%=iif(CInt(CGD_REP_STAT) > 1, " onclick ='return false;'", " onclick=""IfSubSel('SO')"" " )%>>
											<label for="radio8">사회</label>
										</span>
									</p>
									<p class="m_b20">
							...
							<dd>
								<p>
									<span data-form="radio">
										<input type="radio" id="radio9" name="CGD_SUSI_APP_FLG" value="N" <%=iif(iMode="I" Or CGD_SUSI_APP_FLG = "N", "checked", "")%> onclick="IfSUSIFlg('N')" <%=iif(CInt(CGD_REP_STAT) > 1, " disabled='disalbed'", "" )%>>
										<label for="radio9">미접수</label>
									</span>
									<span data-form="radio">
										<input type="radio" id="radio10" name="CGD_SUSI_APP_FLG" value="Y" <%=iif(CGD_SUSI_APP_FLG = "Y", "checked", "")%>  onclick="IfSUSIFlg('Y')" <%=iif(CInt(CGD_REP_STAT) > 1, " disabled='disalbed'", "" )%>>
										<label for="radio10">접수</label>
									</span>
								</p>
								<p>
									<span data-form="input" class="m_b10" id="CGD_SUSI_APP1" >
										<input type="text" id="CGD_SUSI_APP1_TXT" class="ph_small" name="CGD_SUSI_APP1_TXT" placeholder="대학,전형,학부(학과)를 정확하게 입력하세요." value="<%=CGD_SUSI_APP1_TXT%>" <%=iif(iMode="I" Or CGD_SUSI_APP_FLG = "N", " disabled='disabled' ", "" )%> <%=iif(CInt(CGD_REP_STAT) > 1, " readonly='readonly'", "" )%>>
									</span>
								...
								%>
									<span data-form="input" class="full m_b10"  id="CGD_SUSI_APP<%=i%>" class="size390 m_t10" <%=iif(i<=CGD_SUSI_VIEW, "", "style='display:none;'") %>>
										<input type="text" id="CGD_SUSI_APP<%=i%>_TXT" class="ph_small" name="CGD_SUSI_APP<%=i%>_TXT" placeholder="대학,전형,학부(학과)를 정확하게 입력하세요." value="<%=APP_TXT%>" <%=iif(CInt(CGD_REP_STAT) > 1, " readonly='readonly'", "" )%>>
									</span>
								<% Next %>
								</p>
							</dd>
							...
							<dt><span class="field_name">※본 상담은 가채점 점수를 기준으로 한 상담이므로 변동성이 있을 수 있으니 참고 자료로써<br>다른 자료와 종합적으로 판단/활용해 주시길 바랍니다.</span></dt>
